...
const BuyScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  // Função para formatar o número de contato (xxxxx-xxxx)
  const handleContactChange = (value: string) => {
    const formattedContact = value.replace(/\D/g, '').replace(/(\d{5})(\d{4})/, '$1-$2');
    setContact(formattedContact);
  };

  // Função para formatar o número do cartão (xxxx-xxxx-xxxx-xxxx)
  const handleCardNumberChange = (value: string) => {
    const formattedCardNumber = value
      .replace(/\D/g, '')
      .replace(/(\d{4})(?=\d)/g, '$1-')
      .slice(0, 19); // Limita o comprimento ao máximo permitido
  ...
  const handleExpiryDateChange = (value: string) => {
    const formattedExpiryDate = value
      .replace(/\D/g, '')
      .replace(/(\d{2})(\d{2})/, '$1/$2')
      .slice(0, 5); // Limita o comprimento ao máximo permitido
